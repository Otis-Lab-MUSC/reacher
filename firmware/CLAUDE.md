...
- **Lite variants are hand-maintained copies.** A change to a paradigm sketch usually has to be mirrored into its `_lite` twin — 9 sketches, not 5. The `_lite` files differ from their base only by the **Mega-only strip**, so `diff fr/fr.ino fr_lite/fr_lite.ino` should show nothing else. That strip is two-photon (`Microscope` + `Slm`) *plus* `ExternalTrigger`: the UNO has no free external-interrupt pin, since INT0 (2) is the fixed microscope timestamp input and INT1 (3) is the cue output. `ExternalTrigger.cpp` still compiles into lite builds via the shared library, but `--gc-sections` drops it — verified at 91–94% of UNO flash, unchanged by its addition.
- **Serial**: print one JSON object per line, terminated with `\n`. Use the existing level conventions (`000` config / `001` state / `006` error / `007` behavioral / `008` frame / `009` SLM). Level `001` is log-only on the backend **except** for `EXT_TRIGGER`, whose ARMED/DISARMED transitions are firmware-initiated (the trigger self-disarms when it fires) and so are mirrored into `hardware_settings` — see `REACHER.handle_state_event`.
