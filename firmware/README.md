...
> Firmware source lives in the [`reacher`](https://github.com/Otis-Lab-MUSC/reacher) repo under `firmware/`. Compiled hex artifacts are committed to `src/reacher/hex/<board>/` and shipped in the `reacher` PyPI wheel. (The former standalone `Otis-Lab-MUSC/reacher-firmware` repo is archived.)

[![Version](https://img.shields.io/badge/version-2.4.0-blue)](https://github.com/Otis-Lab-MUSC/reacher)
...
```bash
avrdude -p atmega2560 -c wiring -P /dev/ttyUSB0 -b 115200 -U flash:w:../src/reacher/hex/mega/fr.hex:i
```
...
```
reacher/firmware/
├── compile.sh                    # Compiles all 5 paradigms -> ../src/reacher/hex/<board>/
...
├── libraries/
│   └── REACHERDevices/           # Shared device library (v2.4.0)
│       ├── library.properties
│       └── src/
...
Compiled hex binaries are written to ../src/reacher/hex/<board>/ (committed
package data), not into this directory.
```
...
[GitHub: Otis-Lab-MUSC/reacher](https://github.com/Otis-Lab-MUSC/reacher) (firmware under `firmware/`)
